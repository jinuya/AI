...
`SimulatedFeed`는 실시계로 돌 때 **심볼당 초당 1틱**을 낸다(`interval_ns` 기본값). 이건
의도된 속도다 — 매 틱이 1초치 가격 확산을 적용하므로, 스로틀 없이 돌리면 초당 6만 틱이
전부 같은 봉에 들어가 봉 내 변동폭이 수십 %가 되고 fat-finger·가격점프 검사가 인위적으로
발동한다(`atrader.marketdata.feeds.simulated._iterate` 주석 참조). 시뮬레이션 시계에서는
스로틀 없이 즉시 진행하므로 백테스트 속도는 영향받지 않는다.

따라서 봉이 실제로 닫히는 걸 보려면 **실제 시간을 들여야 한다**:

- 1분봉 하나 → 최소 60초 이상 실행
- `sma_crossover`가 신호를 내려면 50봉 워밍업 → 1분봉 기준 약 50분

`bar_intervals: ["1s"]`로 낮추면 봉은 빨리 닫히지만 **틱 하나로 만들어진 봉**(O=H=L=C)이
된다 — 파이프라인이 도는지 확인하는 용도로는 충분하고, 봉 내부 구조가 의미 있어야 하는
검증에는 부적절하다. 실측(심볼 10개, 6초 실행): 틱 60개 / 봉 50개, 즉 봉당 약 1.2틱.
